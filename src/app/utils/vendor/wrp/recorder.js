  ...
  function terminate_() {		
	...
	async function resume_() {
		...
		if (!navigator.mediaDevices) {
			if (recorder_.TRACE) {
        recorder_.TRACE("ERROR: can't start recording (Unsupported MediaDevices class)") ;
      }

			if (recorder_.pauseEnded) {
        recorder_.pauseEnded({code: 2, message: "Unsupported MediaDevices class"}, waveFile_) ;
      }
    ...
    terminate_() ;
		...
		if (audioSamplesPerSec_ == 0) {
			if (recorder_.TRACE) {
        recorder_.TRACE("ERROR: can't start recording (Unsupported sample rate: " + audioContext_.sampleRate + "Hz)") ;
      }
    ...
    var isResumed = false ;
		...
		await navigator.mediaDevices.getUserMedia(
            ...
            recorder_.pauseEnded(reason_, waveFile_) ;
				...
				if (audioDecimatationFactor_ > 1) {
					if (recorder_.TRACE) {
            recorder_.TRACE("INFO: started recording: " + audioContext_.sampleRate + "Hz -> " + audioSamplesPerSec_ + "Hz (" + audioProcessor_.bufferSize + " samples/buffer)") ;
          }
				} else {
					if (recorder_.TRACE) {
            recorder_.TRACE("INFO: started recording: " + audioSamplesPerSec_ + "Hz (" + audioProcessor_.bufferSize + " samples/buffer)") ;
          }
				}
          ...
          recorder_.resumeEnded(((ima_state_ > 0) ? "" : "MSB") + (audioSamplesPerSec_ / 1000 | 0) + "K") ;
        }
        ...
        isResumed = true ;
				...
				if (recorder_.TRACE) {
          recorder_.TRACE("ERROR: can't start recording (" + error.message + ")") ;
        }
	...
	function startMaxRecordingTimeTimer_() {
		...
		if (recorder_.TRACE) {
      recorder_.TRACE("INFO: started auto pause timeout timer: " + recorder_.maxRecordingTime) ;
    }
	}
	...
	function stopMaxRecordingTimeTimer_() {
		if (maxRecordingTimeTimerId_ !== null) {
			...
			if (recorder_.TRACE) {
        recorder_.TRACE("INFO: stopped auto pause timeout timer: " + recorder_.maxRecordingTime) ;
      }
		}
	}
	...
	function fireMaxRecordingTimeTimer_() {
		if (recorder_.TRACE) {
      recorder_.TRACE("INFO: fired auto pause timeout timer: " + recorder_.maxRecordingTime) ;
    }
